...
export const config = {
  api: {
    bodyParser: false, // Disable bodyParser to handle form-data with multer
  },
};
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.headers['content-type'].startsWith('multipart/form-data')) {
      // If the request contains form-data, handle it with multer (for avatar upload)
      ...
      // If the request contains JSON, handle it directly
      let data = '';
      req.on('data', chunk => {
        data += chunk;
      });
