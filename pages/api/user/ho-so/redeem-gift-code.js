...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!userId || !giftCode) {
      return res
        .status(400)
        .json({ message: "User ID and gift code are required" });
    }
    ...
    db.query(userQuery, [userId], async (error, results) => {
      if (error || results.length === 0) {
        return res
          .status(500)
          .json({
            message: "Internal server error or user not found",
            error: error?.message,
          });
      }
      ...
      if (currentTime.diff(moment(userCreatedAt), "days") < 3) {
        return res
          .status(403)
          .json({
            message: "Account must be older than 3 days to redeem a gift code",
          });
      }
      ...
      db.query(checkLogQuery, [userId, giftCode], (logError, logResults) => {
        if (logError) {
          return res
            .status(500)
            .json({
              message: "Internal server error while checking activity log",
        ...
        if (logResults.length > 0) {
          return res
            .status(403)
            .json({ message: "Gift code has already been used by this user" });
        }
        ...
        db.query(codeQuery, [giftCode], (codeError, codeResults) => {
          if (codeError || codeResults.length === 0) {
            return res
              .status(404)
              .json({ message: "Gift code not found or inactive" });
          }
          ...
          if (giftCodeData.time_can_use <= 0) {
            return res
              .status(403)
              .json({ message: "Gift code has already been used" });
          }
          ...
          db.query(
            updateUserQuery,
            [giftCodeData.exp, giftCodeData.tai_san, userId],
            (userUpdateError) => {
              if (userUpdateError) {
                return res
                  .status(500)
                ...
                (giftUpdateError) => {
                  if (giftUpdateError) {
                    return res
                      ...
                      .json({
                        message:
                          "Internal server error while updating gift code",
              ...
              `;
                    ...
                    (itemError, items) => {
                      if (itemError) {
                        return res
                          ...
                          .json({
                            message:
                              "Internal server error while fetching gift items",
                  ...
                  `;
                        db.query(
                          existingItemQuery,
                          [userId, vat_pham_id],
                          (existingItemError, existingItems) => {
                            if (existingItemError) {
                              return res
                                .status(500)
                                .json({
                                  message:
                                    "Internal server error while checking ruong_do",
                            ...
                            if (existingItems.length > 0) {
                              const newQuantity =
                                existingItems[0].so_luong + quantity;
                                ...
                                (updateError) => {
                                  if (updateError) {
                                    return res
                                      ...
                                      .json({
                                        message:
                                          "Internal server error while updating ruong_do",
                                ...
                                (insertError) => {
                                  if (insertError) {
                                    return res
                                      ...
                                      .json({
                                        message:
                                          "Internal server error while inserting into ruong_do",
                  ...
                  VALUES (?, ?, ?, NOW())
                `;
                      const logMessage = `vừa sử dụng gift code ${giftCode} nhận được ${giftCodeData.exp} EXP, ${giftCodeData.tai_san} bạc, và một số vật phẩm đặc biệt.`;
                      const actionType = `giftcode-${giftCode}`;

                      db.query(
                        logQuery,
                        [userId, actionType, logMessage],
                        (logError) => {
                          if (logError) {
                            return res
                              .status(500)
                              .json({
                                message:
                                  "Internal server error while logging activity",
                                error: logError.message,
                              });
                          }
                          ...
                          return res
                            ...
                            .json({
                              ...
                              exp: giftCodeData.exp,
                              tai_san: giftCodeData.tai_san,
                            });
                        }
                      );
                    }
