  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      try {
        ...
        await db.query(updateUserExpQuery, [
          expToAdd,
          now.format("YYYY-MM-DD HH:mm:ss"),
          userId,
        ]);

        const userClanResult = await db.query(
          "SELECT clan_id FROM clan_members WHERE member_id = ?",
          [userId]
        );
        const clanId =
          Array.isArray(userClanResult) && userClanResult.length > 0
            ? userClanResult[0].clan_id
            : null;

        if (clanId) {
          const updateClanManaQuery =
            "UPDATE clans SET clan_mana = CAST(clan_mana AS FLOAT) - ? WHERE id = ?";
        ...
        return res.status(200).json({
          message: "EXP updated and clan mana deducted successfully",
          expAdded: expToAdd,
        });
      } catch (error) {
        await db.query("ROLLBACK");
