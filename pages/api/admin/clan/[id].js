...
export default async function handler(req, res) {
  ...
  if (req.method === "PUT") {
    upload.single("clan_icon")(req, res, async function (err) {
      ...
      const { name, owner, accountant_id, clan_money, clan_mana, clan_color } =
        req.body;
      ...
      try {
        db.query(
          "SELECT owner, accountant_id FROM clans WHERE id = ?",
          [id],
          (selectError, results) => {
            if (selectError) {
              return res
            ...
            db.query(
              ...
              (updateError) => {
                ...
                if (owner && currentOwnerId) {
                  db.query(
                    "UPDATE users SET clan_role = 1 WHERE id = ?",
                    [currentOwnerId],
                    (currentOwnerError) => {
                      if (currentOwnerError) {
                        console.error(
                          "Error setting current owner role to null:",
                          currentOwnerError.message
                        );
                      }
                    }
                  );
                  db.query(
                    "UPDATE users SET clan_role = 7 WHERE id = ?",
                    [owner],
                    (ownerError) => {
                      if (ownerError) {
                        console.error(
                          "Error updating new owner role:",
                          ownerError.message
                        );
                      }
                    }
                  ...
                  db.query(
                    "SELECT id FROM clan_members WHERE clan_id = ? AND member_id = ?",
                    [id, owner],
                    (ownerCheckError, ownerCheckResults) => {
                      if (ownerCheckError) {
                        console.error(
                          "Error checking owner in clan_members:",
                        ...
                        db.query(
                          ...
                          (insertOwnerError) => {
                            if (insertOwnerError) {
                              console.error(
                                "Error inserting new owner into clan_members:",
                                insertOwnerError.message
                              );
                            }
                          }
                        );
                ...
                if (accountant_id && currentAccountantId) {
                  db.query(
                    "UPDATE users SET clan_role = 1 WHERE id = ?",
                    [currentAccountantId],
                    (currentAccountantError) => {
                      if (currentAccountantError) {
                        console.error(
                          "Error setting current accountant role to null:",
                          currentAccountantError.message
                        );
                      }
                    }
                  );
                  db.query(
                    ...
                    (accountantError) => {
                      if (accountantError) {
                        console.error(
                          "Error updating new accountant role:",
                          accountantError.message
                        );
                      }
                    }
                  );
                  db.query(
                    "SELECT id FROM clan_members WHERE clan_id = ? AND member_id = ?",
                    [id, accountant_id],
                    (accountantCheckError, accountantCheckResults) => {
                      if (accountantCheckError) {
                        console.error(
                          "Error checking accountant in clan_members:",
                          accountantCheckError.message
                        );
                      } else if (accountantCheckResults.length === 0) {
                        db.query(
                          "INSERT INTO clan_members (clan_id, member_id) VALUES (?, ?)",
                          [id, accountant_id],
                          (insertAccountantError) => {
                            if (insertAccountantError) {
                              console.error(
                                "Error inserting new accountant into clan_members:",
                                insertAccountantError.message
                              );
                            }
                          }
                        );
              ...
              (deleteRequestsError) => {
                if (deleteRequestsError) {
                  ...
                  return res
                    ...
                    .json({
                      message: "Internal server error",
                      error: deleteRequestsError.message,
                    });
                }
                db.query(
                  "DELETE FROM clan_activity_logs WHERE clan_id = ?",
                  [id],
                  (deleteActivityLogsError) => {
                    ...
                    db.query(
                      ...
                      (deleteClanError) => {
                        ...
                        db.query("COMMIT", (commitError) => {
                          ...
                          res
                            .status(200)
                            .json({ message: "Clan deleted successfully" });
                        });
                      }
                    );
                  }
